...
- During feature `fix-t24-live-features`, the default live pipeline path was widened back to full same-day inference assembly: `src/pipeline/daily.py` now routes through `build_live_feature_frame()`, which rebuilds offense, pitching, defense, bullpen, baseline, and weather-backed inputs from the current schedule plus historical context and ignores stale persisted feature rows for live runs.
- The daily pipeline now treats missing open-air weather the same as missing lineups/odds for validation purposes, persisting explicit `NO PICK` rows such as `weather unavailable` or `odds unavailable; weather unavailable` rather than silently falling back to neutral weather in recommendation output.

- During feature `t31-error-handling`, the manifest command `commands.init_windows` in `.factory/services.yaml` still printed a Python `SyntaxError` from the `init_db(''data/mlb.db'')` inline command because of doubled single-quote escaping, even though the overall setup command exited 0 after installing dependencies and creating data directories. Future workers should run `src.db.init_db()` directly if they need a guaranteed database init until the manifest command is corrected.
