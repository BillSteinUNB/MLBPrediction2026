...
- During feature `t24-daily-orchestrator`, the default live pipeline path was narrowed to fast inference assembly: it now reuses any already-persisted feature rows, computes only the lightweight baseline features on-demand, and relies on leakage-safe fill defaults for missing model columns instead of re-running the full offense/pitching/defense/bullpen feature stack at CLI runtime.
