...
def test_compute_weather_adjustment_ignores_postgame_weather_snapshots() -> None:
    scheduled_start = datetime(2025, 4, 10, 19, 0, tzinfo=timezone.utc)
    result = compute_weather_adjustment(
        _weather(
            temperature_f=88.0,
            humidity_pct=72.0,
            ...
            update={
                "forecast_time": datetime(2025, 4, 10, 20, 0, tzinfo=timezone.utc),
                "fetched_at": datetime(2025, 4, 10, 18, 0, tzinfo=timezone.utc),
            }
        ),
        team_code="NYY",
