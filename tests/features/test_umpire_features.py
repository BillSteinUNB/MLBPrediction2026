...
def test_compute_umpire_features_emits_abs_active_history_splits(tmp_path: Path) -> None:
    db_path = tmp_path / "umpires_abs.db"
    _seed_game(
        db_path,
        game_pk=1101,
        game_date="2024-04-01",
    ...
    def _fake_umpire_fetcher(*, season: int, refresh: bool = False) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            [
                {"date": "2024-04-01", "hometeam": "NYA", "visteam": "BOS", "umphome": "westj902"},
                {"date": "2024-04-02", "hometeam": "NYA", "visteam": "BOS", "umphome": "westj902"},
                {"date": "2024-04-03", "hometeam": "NYA", "visteam": "BOS", "umphome": "westj902"},
            ]
        )
    ...
    feature_map = {feature.feature_name: feature.feature_value for feature in features if feature.game_pk == 1103}

    assert feature_map["plate_umpire_sample_size_30g"] == 2.0
    assert feature_map["plate_umpire_abs_active_share_30g"] == pytest.approx(0.5)
    assert feature_map["plate_umpire_abs_active_sample_size_30g"] == 1.0
    assert feature_map["plate_umpire_abs_total_runs_avg_30g"] == pytest.approx(8.0)
    assert feature_map["plate_umpire_abs_f5_total_runs_avg_30g"] == pytest.approx(3.0)
