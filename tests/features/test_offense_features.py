        ...
        2025: pd.DataFrame(
            {
                ...
                "pa": [4, 5, 4, 4, 4, 4, 4, 4],
                "bbe": [2, 1, 2, 2, 1, 2, 2, 1],
                "bat_tracking_events": [3, 5, 2, 4, 2, 4, 3, 3],
                "xwoba": [0.39, 0.27, 0.34, 0.32, 0.31, 0.30, 0.28, 0.26],
                "barrel_pct": [50.0, 0.0, 50.0, 0.0, 0.0, 50.0, 0.0, 0.0],
                "bat_speed": [71.0, 68.0, 72.0, 70.0, 67.0, 66.0, 68.0, 67.5],
                "swing_length": [7.4, 7.0, 7.6, 7.2, 6.9, 7.1, 7.0, 6.8],
                "swing_path_tilt": [33.0, 31.0, 34.0, 30.0, 32.0, 31.5, 32.5, 31.0],
                "squared_up_rate_proxy": [50.0, 0.0, 50.0, 25.0, 0.0, 50.0, 0.0, 0.0],
                "bat_tracking_coverage": [0.75, 1.0, 0.5, 1.0, 0.5, 1.0, 0.75, 0.75],
            }
        ),
        2024: pd.DataFrame(
            {
                ...
                "pa": [5, 4, 4, 4],
                "bbe": [3, 1, 2, 2],
                "bat_tracking_events": [4, 3, 2, 2],
                "xwoba": [0.33, 0.29, 0.31, 0.30],
                "barrel_pct": [33.3333333333, 0.0, 0.0, 50.0],
                "bat_speed": [70.5, 67.5, 66.0, 65.5],
                "swing_length": [7.3, 6.9, 6.8, 6.7],
                "swing_path_tilt": [32.5, 30.5, 31.0, 30.0],
                "squared_up_rate_proxy": [33.3333333333, 0.0, 0.0, 50.0],
                "bat_tracking_coverage": [0.8, 0.75, 0.5, 0.5],
            }
        ),
    }
...
def test_compute_offensive_features_integrates_bat_tracking_metrics_from_statcast(
    tmp_path: Path,
) -> None:
    from src.features.offense import compute_offensive_features

    db_path = tmp_path / "offense_bat_tracking.db"
    init_db(db_path)
    _seed_game(
        db_path,
        game_pk=563,
    ...
    current_bat_speed = (((71.0 * 3 + 68.0 * 5) / 8) + ((72.0 * 2 + 70.0 * 4) / 6)) / 2
    current_swing_length = (((7.4 * 3 + 7.0 * 5) / 8) + ((7.6 * 2 + 7.2 * 4) / 6)) / 2
    current_squared_up = ((((50.0 * 2) + (0.0 * 1)) / 3) + (((50.0 * 2) + (25.0 * 2)) / 4)) / 2
    current_coverage = (((0.75 * 4) + (1.0 * 5)) / 9 + ((0.5 * 4) + (1.0 * 4)) / 8) / 2
    prior_bat_speed = (70.5 * 4 + 67.5 * 3) / 7
    prior_swing_length = (7.3 * 4 + 6.9 * 3) / 7
    prior_squared_up = ((33.3333333333 * 3) + (0.0 * 1)) / 4
    prior_coverage = ((0.8 * 5) + (0.75 * 4)) / 9
    expected_bat_speed = (current_bat_speed * 2 + prior_bat_speed * 30) / 32
    expected_swing_length = (current_swing_length * 2 + prior_swing_length * 30) / 32
    expected_squared_up = (current_squared_up * 2 + prior_squared_up * 30) / 32
    expected_coverage = (current_coverage * 2 + prior_coverage * 30) / 32
    ...
    by_name = {row.feature_name: row.feature_value for row in rows}
    assert by_name["home_team_bat_speed_2g"] == pytest.approx(expected_bat_speed)
    assert by_name["home_team_swing_length_2g"] == pytest.approx(expected_swing_length)
    assert by_name["home_team_squared_up_rate_proxy_2g"] == pytest.approx(expected_squared_up)
    assert by_name["home_team_bat_tracking_coverage_2g"] == pytest.approx(expected_coverage)
    assert by_name["home_lineup_bat_speed_2g"] == pytest.approx(by_name["home_team_bat_speed_2g"])


def test_compute_offensive_features_marks_bat_tracking_fallback_when_metric_columns_are_missing(
    tmp_path: Path,
) -> None:
    ...
    db_path = tmp_path / "offense_bat_tracking_fallback.db"
    init_db(db_path)
    _seed_game(
        db_path,
        game_pk=564,
    ...
    def fake_offense_statcast_fetcher(
        ...
        *,
        ...
        _ = refresh
        return pd.DataFrame(
            columns=["game_pk", "game_date", "team", "player_id", "pa", "bbe", "xwoba", "barrel_pct"]
        )
    ...
    by_name = {row.feature_name: row.feature_value for row in rows}
    assert by_name["home_team_bat_speed_2g"] == pytest.approx(LEAGUE_BAT_SPEED_BASELINE)
    assert by_name["home_team_squared_up_rate_proxy_2g"] == pytest.approx(
        LEAGUE_SQUARED_UP_RATE_PROXY_BASELINE
    )
    assert by_name["home_team_bat_tracking_coverage_2g"] == pytest.approx(
        LEAGUE_BAT_TRACKING_COVERAGE_BASELINE
    )
