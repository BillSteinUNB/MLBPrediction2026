...
def test_build_training_dataset_carries_bat_tracking_features_into_training_frame(
    tmp_path: Path,
) -> None:
    schedule = pd.DataFrame(
        [
    ...
    statcast_by_season = {
        2025: pd.DataFrame(
            {
                "game_pk": [4099, 4099, 4100, 4100, 4199, 4199, 4200, 4200],
                "game_date": [
                    "2025-03-31",
                    "2025-03-31",
                    "2025-04-01",
        ...
        2024: pd.DataFrame(
            {
                "game_pk": [9001, 9001, 9002, 9002],
                "game_date": ["2024-09-15", "2024-09-15", "2024-09-15", "2024-09-15"],
    ...
    dataset = build_training_dataset(
        start_year=2025,
        end_year=2025,
        output_path=tmp_path / "bat_tracking_sample.parquet",
        full_regular_seasons_target=1,
        shortened_season_game_threshold=0,
        schedule_fetcher=lambda year: schedule.copy(),
        batting_stats_fetcher=lambda *_args, **_kwargs: pd.DataFrame(),
        ...
        lineup_fetcher=lambda *_args, **_kwargs: [],
        weather_fetcher=_fake_weather_fetcher,
        offense_statcast_fetcher=lambda season, **_kwargs: statcast_by_season[season].copy(),
    ).dataframe

    row = dataset.iloc[0]

    assert "home_team_bat_speed_7g" in dataset.columns
    assert "home_team_swing_length_7g" in dataset.columns
    assert "home_team_squared_up_rate_proxy_7g" in dataset.columns
    assert "home_team_bat_tracking_coverage_7g" in dataset.columns
    assert row["home_team_bat_speed_7g"] > 69.25
    assert row["home_team_bat_tracking_coverage_7g"] > 0.0
    assert row["home_team_squared_up_rate_proxy_7g"] > 18.0
...
def test_build_live_feature_frame_keeps_same_day_postponed_games_when_official_date_rolls(
    tmp_path: Path,
) -> None:
    current_schedule = pd.DataFrame(
        [
            {
                **_schedule_row(
                    ...
                    home_starter_id=123456,
                    away_starter_id=234567,
                    status="postponed",
                    f5_home_score=0,
                    f5_away_score=0,
                    final_home_score=0,
    ...
    frame = build_live_feature_frame(
        ...
        schedule=current_schedule,
        historical_games=pd.DataFrame(),
        db_path=tmp_path / "live_postponed.db",
        lineups=[],
        weather_fetcher=_fake_weather_fetcher,
        batting_stats_fetcher=lambda *_args, **_kwargs: pd.DataFrame(),
        team_logs_fetcher=_fake_team_logs_fetcher({}),
    ...
    assert frame["game_pk"].tolist() == [824621]
    assert frame.iloc[0]["scheduled_start"] == "2026-04-02T20:10:00+00:00"
