...
def test_run_daily_pipeline_prod_rechecks_inputs_and_neutralizes_missing_umpire(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "prod_refresh.db"
    initial_schedule = _schedule_frame(game_pks=[1001])
    refreshed_schedule = initial_schedule.copy()
    refreshed_schedule.loc[0, "home_starter_id"] = 9001

    initial_lineups = _make_lineups([1001])
    refreshed_lineups = [lineup.model_copy(deep=True) for lineup in _make_lineups([1001])]
    refreshed_lineups[0] = refreshed_lineups[0].model_copy(
        update={
            "as_of_timestamp": datetime(2025, 9, 15, 17, 45, tzinfo=UTC),
            "starting_pitcher_id": 9001,
            "projected_starting_pitcher_id": 9001,
            "players": [
                refreshed_lineups[0].players[0].model_copy(update={"player_id": 999001}),
                *refreshed_lineups[0].players[1:],
            ],
        }
    )

    class _CapturingPredictionEngine:
        model_version = "test-model-v1"
        ...
        def predict(self, inference_frame: pd.DataFrame) -> Prediction:
            self.frames.append(inference_frame.copy())
            return _prediction(1001, ml_home=0.64, rl_home=0.58)

    refresh_flags: list[bool] = []
    schedule_calls = {"count": 0}
    lineup_calls = {"count": 0}

    def _schedule_fetcher(target_date, mode):
        del target_date, mode
        schedule_calls["count"] += 1
        if schedule_calls["count"] == 1:
            return initial_schedule.copy()
        return refreshed_schedule.copy()

    def _lineups_fetcher(target_date: str):
        del target_date
        lineup_calls["count"] += 1
        if lineup_calls["count"] == 1:
            return initial_lineups
        return refreshed_lineups

    def _feature_frame_builder_with_refresh(**kwargs: object) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            [
                {
                    "game_pk": int(schedule.iloc[0]["game_pk"]),
                    "scheduled_start": schedule.iloc[0]["scheduled_start"],
                    "home_team": schedule.iloc[0]["home_team"],
                    "away_team": schedule.iloc[0]["away_team"],
                    "venue": schedule.iloc[0]["venue"],
                    "park_runs_factor": 1.02,
                    "park_hr_factor": 1.01,
                    "home_team_f5_pythagorean_wp_30g": 0.56,
                    "home_team_log5_30g": 0.57,
                    "weather_composite": 1.12 if refresh else 0.98,
                    "weather_wind_factor": 8.0 if refresh else 2.0,
                    "weather_data_missing": 0.0,
                    "as_of_timestamp": (
                        "2025-09-15T17:45:00+00:00" if refresh else "2025-09-15T17:00:00+00:00"
                    ),
                    "plate_umpire_known": 0.0,
                    "plate_umpire_sample_size_30g": 14.0,
                    "plate_umpire_total_runs_avg_30g": 9.7,
                    "plate_umpire_f5_total_runs_avg_30g": 5.1,
                }
            ]
        )

    notifier = _RecordingNotifier()
    prediction_engine = _CapturingPredictionEngine()
    dependencies = PipelineDependencies(
        schedule_fetcher=_schedule_fetcher,
        history_fetcher=lambda season, before_date: pd.DataFrame(),
        lineups_fetcher=_lineups_fetcher,
        odds_fetcher=lambda target_date, mode, db_path: [
            _odds_snapshot(1001, market_type="f5_ml", home_odds=-110, away_odds=100)
        ],
        full_game_odds_fetcher=lambda target_date, mode, db_path: {},
        feature_frame_builder=_feature_frame_builder_with_refresh,
        prediction_engine=prediction_engine,
        notifier=notifier,
    )
    ...
    assert result.pick_count == 1
    assert len(prediction_engine.frames) == 1
    assert prediction_engine.frames[0].iloc[0]["weather_composite"] == pytest.approx(1.12)
    assert prediction_engine.frames[0].iloc[0]["plate_umpire_sample_size_30g"] == 0.0
    assert prediction_engine.frames[0].iloc[0]["plate_umpire_total_runs_avg_30g"] == pytest.approx(8.8)

    input_status = result.games[0].input_status
    assert input_status is not None
    assert input_status["prelock_recheck_performed"] is True
    assert input_status["starter_changed_during_recheck"] is True
    assert input_status["lineup_changed_during_recheck"] is True
    assert input_status["weather_changed_during_recheck"] is True
    assert input_status["umpire_available"] is False
    assert input_status["umpire_neutralized"] is True
    assert input_status["umpire_fallback_mode"] == "neutralized_defaults"


def test_run_daily_pipeline_prod_records_after_refresh_weather_block_reason(tmp_path: Path) -> None:
    db_path = tmp_path / "weather_after_refresh.db"
    schedule = _schedule_frame(game_pks=[1001])
    notifier = _RecordingNotifier()
    prediction_engine = _RecordingPredictionEngine({1001: _prediction(1001, ml_home=0.58, rl_home=0.53)})
    ...
    assert result.pick_count == 0
    assert result.no_pick_count == 1
    assert result.games[0].no_pick_reason == "weather unavailable after refresh"
    assert result.games[0].input_status is not None
    assert result.games[0].input_status["prelock_recheck_performed"] is True
    assert result.games[0].input_status["weather_refresh_required"] is True
    ...
    assert stored_reason == "weather unavailable after refresh"
...
def test_official_selector_rejects_estimated_books() -> None:
    estimated_candidate = BetDecision(
        game_pk=1001,
        market_type="f5_ml",
        side="away",
        source_model="legacy_f5_ml",
        source_model_version="test-model-v1",
        book_name="estimate:full-game:BetMGM",
    ...
    selected, kill_switch = _select_game_decision(
        candidates=[estimated_candidate, real_candidate],
        current_bankroll=100.0,
        peak_bankroll=100.0,
    )
    ...
    assert kill_switch is False
    assert selected is not None
    assert selected.book_name == "BetMGM"
    assert selected.side == "home"


def test_official_live_candidate_matches_researched_full_game_policy() -> None:
    valid_ml = BetDecision(
        game_pk=1001,
    ...
    assert _is_official_live_candidate(valid_ml) is True

    low_edge = valid_ml.model_copy(update={"edge_pct": 0.149})
    assert _select_game_decision(
        candidates=[low_edge],
        current_bankroll=100.0,
        peak_bankroll=100.0,
    )[0] is None

    too_high_edge = valid_ml.model_copy(update={"edge_pct": 0.225})
    assert _is_official_live_candidate(too_high_edge) is False

    f5_candidate = valid_ml.model_copy(
        update={"market_type": "f5_ml", "source_model": "run_count_f5_ml"}
    )
    assert _is_official_live_candidate(f5_candidate) is False

    rl_bad_price = valid_ml.model_copy(
        update={
    ...
    assert _is_official_live_candidate(rl_bad_price) is False

    rl_good_price = valid_ml.model_copy(
        update={
            "market_type": "full_game_rl",
            "source_model": "run_count_full_game_rl",
            "odds_at_bet": 100,
    ...
    assert _is_official_live_candidate(rl_good_price) is True
