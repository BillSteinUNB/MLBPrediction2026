...
def test_load_pending_bets_uses_persisted_decision_metadata(tmp_path: Path) -> None:
    from src.engine.settlement import _load_pending_bets

    db_path = tmp_path / "pending_bets.db"
    init_db(db_path)
    _seed_game(db_path, 12345)

    placed_decision = BetDecision(
        game_pk=12345,
        market_type="f5_total",
        side="over",
        book_name="bet365",
        source_model="run_count",
        source_model_version="reset-test-1",
    ...
    with sqlite3.connect(db_path) as connection:
        pending_bets = _load_pending_bets(connection, 12345)

    assert len(pending_bets) == 1
    pending_bet = pending_bets[0]
    assert pending_bet.book_name == "bet365"
    assert pending_bet.source_model == "run_count"
    assert pending_bet.source_model_version == "reset-test-1"
    assert pending_bet.model_probability == pytest.approx(0.61)
    assert pending_bet.fair_probability == pytest.approx(0.56)
    assert pending_bet.ev == pytest.approx(0.14)
    assert pending_bet.line_at_bet == pytest.approx(4.5)
