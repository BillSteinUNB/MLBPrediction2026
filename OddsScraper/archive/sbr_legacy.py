"""
Legacy MLB historical odds scraper for SportsbookReview.com
...
class OddsRow:
    ...
    fetched_at: str = ""
    bookmaker: str = ""
    market_type: str = ""  # full_game_ml, full_game_rl, full_game_total, f5_ml, f5_rl, f5_total, full_game_team_total_home, full_game_team_total_away
...
BETUS_BOARD_URL = "https://www.betus.com.pa/sportsbook/mlb/"
BETUS_BOOKMAKER = "BetUS"
BETUS_TEAM_NAME_TO_CODE = {
...
EXTRACT_BETUS_MLB_BOARD_JS = r"""
() => {
    const clean = (value) => (value || "").replace(/\s+/g, " ").trim();
    const boardDateText =
        clean(document.querySelector("#sport-title .date")?.textContent) ||
        clean(document.querySelector(".date.font-weight-normal")?.textContent);
    const games = Array.from(document.querySelectorAll(".game-tbl.container-fluid.mb-1")).map((card) => {
        const awayBlock = card.querySelector(".visitor-lines");
        const homeBlock = card.querySelector(".home-lines");
        const text = (root, selector) => clean(root?.querySelector(selector)?.textContent);
        return {
            boardDateText,
            gameTimeText: clean(card.querySelector(".time")?.textContent),
            matchupHref: card.querySelector("a.link-matchup")?.href || "",
            marketHref: card.querySelector("a.link-markets")?.href || "",
            awayRotation: text(awayBlock, ".away-rotation"),
            homeRotation: text(homeBlock, ".home-rotation"),
            awayTeam: text(awayBlock, "#awayName a, #awayName, .team a, .team"),
            homeTeam: text(homeBlock, "#homeName a, #homeName, .team a, .team"),
            awayTeamTotalOverText: text(awayBlock, "a[id*='lnkBetOverVisitor']"),
            awayTeamTotalUnderText: text(awayBlock, "a[id*='lnkBetUnderVisitor']"),
            homeTeamTotalOverText: text(homeBlock, "a[id*='lnkBetHomeOver']"),
            homeTeamTotalUnderText: text(homeBlock, "a[id*='lnkBetHomeUnder']"),
        };
    }).filter((game) =>
        game.awayTeam &&
        game.homeTeam &&
        (game.awayTeamTotalOverText || game.awayTeamTotalUnderText || game.homeTeamTotalOverText || game.homeTeamTotalUnderText)
    );
    return { boardDateText, games };
}
"""
...
class MLBOddsScraper:
    ...
    @staticmethod
    def _normalize_betus_offer_text(raw: str) -> str:
        text = raw or ""
        for source, target in BETUS_FRACTION_REPLACEMENTS.items():
            text = text.replace(source, target)
        text = text.replace("Added", "")
        return re.sub(r"\s+", " ", text).strip()

    @classmethod
    def _parse_betus_team_code(cls, raw: str) -> str:
        text = re.sub(r"\s+", " ", raw or "").strip()
        if not text:
            return ""
        if text in BETUS_TEAM_NAME_TO_CODE:
            return BETUS_TEAM_NAME_TO_CODE[text]
        upper = text.upper()
        return upper if upper in TEAM_ABBREVS else text

    @classmethod
    def _parse_betus_total_offer(cls, raw: str) -> tuple[Optional[float], Optional[int]]:
        text = cls._normalize_betus_offer_text(raw)
        if not text:
            return None, None
        match = re.search(r"(?:[OU]\s*)?([+-]?\d+(?:\.\d+)?)\s*([+-]\d{2,4})$", text)
        if not match:
            return None, None
        return cls._parse_point(match.group(1)), cls._parse_price(match.group(2))

    @staticmethod
    def _parse_betus_board_date(board_date_text: str) -> tuple[Optional[str], str]:
        text = re.sub(r"\s+", " ", board_date_text or "").strip()
        match = re.search(r"[A-Za-z]{3},\s+([A-Za-z]{3})\s+(\d{1,2}),\s+(\d{4})(?:\s+([A-Z]{2,4}))?$", text)
        if not match:
            return None, "EST"
        month_abbrev, day, year, timezone_abbrev = match.groups()
        parsed = datetime.strptime(f"{month_abbrev} {day} {year}", "%b %d %Y")
        return parsed.strftime("%Y-%m-%d"), timezone_abbrev or "EST"

    @staticmethod
    def _extract_betus_event_id(matchup_href: str, market_href: str, game_date: str, away_team: str, home_team: str) -> str:
        match = re.search(r"/matchup/(\d+)/", matchup_href or "")
        if match:
            return f"betus-{match.group(1)}"
        if market_href:
            slug = market_href.rstrip("/").rsplit("/", 1)[-1]
            return f"betus-{game_date}-{slug}"
        return f"betus-{game_date}-{away_team}-{home_team}"

    def _build_betus_team_total_rows(self, payload: dict, fetched_at: Optional[str] = None) -> list[OddsRow]:
        board_date_text = str(payload.get("boardDateText") or "")
        game_date, timezone_abbrev = self._parse_betus_board_date(board_date_text)
        if not game_date:
            return []

        fetched_timestamp = fetched_at or datetime.now(timezone.utc).isoformat()
        parsed_date = datetime.strptime(game_date, "%Y-%m-%d")
        game_type = self.classify_date(parsed_date) or ""
        rows: list[OddsRow] = []

        for game in payload.get("games", []):
            away_team = self._parse_betus_team_code(str(game.get("awayTeam") or ""))
            home_team = self._parse_betus_team_code(str(game.get("homeTeam") or ""))
            if not away_team or not home_team:
                continue

            game_time_text = str(game.get("gameTimeText") or "").replace("\xa0", " ").strip()
            commence_time_utc = self._parse_time_to_utc(game_date, f"{game_time_text} {timezone_abbrev}".strip())
            event_id = self._extract_betus_event_id(
                str(game.get("matchupHref") or ""),
                str(game.get("marketHref") or ""),
                game_date,
                away_team,
                home_team,
            )
            ...
            row_specs = (
                ("awayTeamTotalOverText", "full_game_team_total_away", "over"),
                ("awayTeamTotalUnderText", "full_game_team_total_away", "under"),
                ("homeTeamTotalOverText", "full_game_team_total_home", "over"),
                ("homeTeamTotalUnderText", "full_game_team_total_home", "under"),
            )
            for field_name, market_type, side in row_specs:
                point, price = self._parse_betus_total_offer(str(game.get(field_name) or ""))
    ...
    async def scrape_betus_team_totals(self, context: BrowserContext) -> list[OddsRow]:
        """Scrape the current BetUS MLB board for full-game away/home team totals."""
        page = await context.new_page()
        try:
            logger.info("Scraping BetUS MLB board for team totals...")
            response = await page.goto(BETUS_BOARD_URL, wait_until="networkidle", timeout=120000)
            if response is None or response.status >= 400:
                raise RuntimeError(f"BetUS board request failed with status {response.status if response else 'unknown'}")
            await page.wait_for_selector(".game-tbl.container-fluid.mb-1", timeout=30000)
            payload = await page.evaluate(EXTRACT_BETUS_MLB_BOARD_JS)
            rows = self._build_betus_team_total_rows(payload)
            logger.info(f"BetUS team totals: {len(payload.get('games', []))} games, {len(rows)} rows")
            return rows
        finally:
            await page.close()
...
class SQLiteStore:
    ...
    def _migrate_legacy_schema(self) -> None:
        odds_columns = self._table_columns("odds")
        if not odds_columns:
            return

        if "commence_time_utc" not in odds_columns:
            self._conn.execute("ALTER TABLE odds ADD COLUMN commence_time_utc TEXT")
            if "commence_time" in odds_columns:
                self._conn.execute(
                    """
                    UPDATE odds
                    SET commence_time_utc = commence_time
                    WHERE commence_time_utc IS NULL
                      AND commence_time IS NOT NULL
                    """
                )

        if "is_opening" not in odds_columns:
            self._conn.execute("ALTER TABLE odds ADD COLUMN is_opening INTEGER DEFAULT 0")
            self._conn.execute("UPDATE odds SET is_opening = COALESCE(is_opening, 0)")

        if "game_id" not in odds_columns:
            self._conn.execute("ALTER TABLE odds ADD COLUMN game_id INTEGER")

        games_columns = self._table_columns("games")
        if games_columns and self._conn.execute("SELECT COUNT(*) FROM games").fetchone()[0] == 0:
            source_commence_column = (
                "commence_time_utc" if "commence_time_utc" in self._table_columns("odds") else "commence_time"
            )
            self._conn.execute(
                f"""
            ...
            self._conn.execute(
                ...
                SET game_id = (
                    ...
                    FROM games
                    WHERE games.event_id = odds.event_id
                )
    ...
    def flag_first_seen_rows_as_opening(self, rows: list[OddsRow]) -> None:
        """Mark first-seen rows as opening snapshots for incremental sources like BetUS."""
        seen_in_batch: set[tuple[str, str, str, str, float | None]] = {
            (
                row.event_id,
                row.bookmaker,
                row.market_type,
        ...
        for row in rows:
            ...
            existing = self._conn.execute(
                ...
                WHERE event_id = ?
                  ...
                  AND market_type = ?
                  AND side = ?
                  AND COALESCE(point, -999.0) = COALESCE(?, -999.0)
                LIMIT 1
                """,
                key,
            ).fetchone()
    ...
    def get_oldest_game_date_for_bookmaker_prefix(
        self,
        *,
        bookmaker_prefix: str,
        season: int | None = None,
    ) -> Optional[str]:
        sql = "SELECT MIN(game_date) FROM odds WHERE bookmaker LIKE ?"
        params: list[object] = [f"{bookmaker_prefix}%"]
        if season is not None:
            sql += " AND game_date >= ? AND game_date < ?"
            params.extend([f"{season:04d}-01-01", f"{season + 1:04d}-01-01"])
        row = self._conn.execute(sql, params).fetchone()
        return row[0] if row and row[0] else None

    def get_existing_event_ids_for_bookmaker_prefix(
        self,
        *,
        bookmaker_prefix: str,
        season: int | None = None,
    ) -> set[str]:
        sql = "SELECT DISTINCT event_id FROM odds WHERE bookmaker LIKE ?"
        params: list[object] = [f"{bookmaker_prefix}%"]
        if season is not None:
            sql += " AND game_date >= ? AND game_date < ?"
            params.extend([f"{season:04d}-01-01", f"{season + 1:04d}-01-01"])
